                                              ...
                                              snd_device_t snd_device)
{
    ...
    list_for_each(node, &adev->usecase_list) {
        usecase = node_to_item(node, struct audio_usecase, list);
         ...
         ALOGD("%s:becf: (%d) check_usecases curr device: %s, usecase device:%s "	         ALOGD("%s:becf: (%d) check_usecases curr device: %s, usecase device:%s "
             "backends match %d",__func__, i,	             "backends match %d",__func__, i,
               platform_get_snd_device_name(snd_device),	               platform_get_snd_device_name(snd_device),
               platform_get_snd_device_name(usecase->out_snd_device),	               platform_get_snd_device_name(usecase->out_snd_device),
               platform_check_backends_match(snd_device, usecase->out_snd_device));	+              match);
         if ((usecase->type != PCM_CAPTURE) && (usecase != uc_info)) {	+        if (match && (usecase->type != PCM_CAPTURE) && (usecase != uc_info)) {
             uc_derive_snd_device = derive_playback_snd_device(adev->platform,	             uc_derive_snd_device = derive_playback_snd_device(adev->platform,
                                                usecase, uc_info, snd_device);	                                                usecase, uc_info, snd_device);
